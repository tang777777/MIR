...
class DisDataLoader(LightningDataModule):
    ...
    def __init__(
        ...
        """
        positive_file: the third column ('smiles') of the original preprocessed data
    ...
    def setup(self):
        ...
        self.positive_data = pd.read_csv(self.positive_file, sep=';', names=['scaffold', 'decorations', 'smiles'])
        self.negative_data = pd.read_csv(self.negative_file, sep=';', names=['scaffold', 'decorations', 'smiles'])

        
        min_size = min(len(self.positive_data), len(self.negative_data))
        if len(self.positive_data) > len(self.negative_data):
            self.positive_data = self.positive_data.sample(n=min_size, random_state=42)
        else:
            self.negative_data = self.negative_data.sample(n=min_size, random_state=42)
